...
function deploy(server, target) {
	...
	try { exec('ssh -i deploy.key services@%s "pm2 delete %s-web"', server, target); } catch(e) {}
...
gulp.task('live', [], function(done) { // 'compile'
	var servers = ['web1.mtel.tria.hr'] // , 'web2.mtel.tria.hr']
